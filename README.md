...
> Note: If CloudVision does not have Internet access the file would have to be downloaded to a local machine and uploaded
> to the CloudVision server(s).

2\. Modify the snmpd.conf files to match your requirements.
...
3\. Copy the snmpd.conf file (note this will automatically copy the files to all nodes in case of a multinode setup)

```shell
make copy_snmp_config
```

4\. Install the files (note this will automatically copy the files to all nodes in case of a multinode setup)

On CVP versions older than 2022.3.0:

```shell
make install-old
```

On CVP versions newer than 2022.3.0:
...
```shell
cvpi -v=3 start cvp-snmp
```

6\. Validation

From the CVP server, we can verify the status of the pods, deployment and service with `kubectl get all -l app=snmpd-monitor`

Expected output:

```shell
$ kubectl get all -l app=snmpd-monitor
NAME                      READY   STATUS    RESTARTS   AGE
pod/snmpd-monitor-2vwtw   1/1     Running   0          99m
pod/snmpd-monitor-n8mqk   1/1     Running   0          99m
pod/snmpd-monitor-smmjg   1/1     Running   0          99m

NAME                    TYPE       CLUSTER-IP    EXTERNAL-IP   PORT(S)         AGE
service/snmpd-monitor   NodePort   10.42.137.7   <none>        161:30161/UDP   84m

NAME                           DESIRED   CURRENT   READY   UP-TO-DATE   AVAILABLE   NODE SELECTOR   AGE
daemonset.apps/snmpd-monitor   3         3         3       3            3           <none>          99m
```

7\. Install a cron entry
